...
const TableOne = () => {
    ...
    useEffect(() => {
        ...
        fetch(apiUrl + "/v1/get-loan1", requestOptions)
    ...
    const Seen = (data) => {
        ...
        fetch(apiUrl+"/v1/seen-loan1/"+data._id, requestOptions)
            .then(response => response.json())
            .then(result => {
                if(result.success){
    ...
    const clickNextPage = (data) => {
        return (event) => {
            ...
            window.open((window.location.href).split('/')[0] + '//' + (window.location.href).split('/')[2] + "/loan1-inbox/" + data._id);
            Seen(data)
        }
    }
    ...
    const clickDel = (data) => {
        return (event) => {
            ...
            fetch(apiUrl + "/v1/del-loan1/" + data._id, requestOptions)
                .then(response => response.json())
                .then(result => {
                    if (result.success) {
